...
from resdp.dump_utils import DUMP_ROUTINES, dump
            ...
            (
                ...
                (
                    pd.DataFrame(
                        ...
                        columns=DATA_DIRECTORY["SWOF"].specification.columns,
                    ).set_index(
                        DATA_DIRECTORY["SWOF"].specification.columns[
                            DATA_DIRECTORY["SWOF"].specification.domain[0]
                        ]
                    ),
                    pd.DataFrame(
                        np.array(
            ...
            (
                ...
                (
                    pd.DataFrame(
                        ...
                        columns=DATA_DIRECTORY["PVTO"].specification.columns,
                    ).set_index(
                        [
                            DATA_DIRECTORY["PVTO"].specification.columns[i]
                            for i in DATA_DIRECTORY["PVTO"].specification.domain
                        ]
                    ),
                    pd.DataFrame(
                        np.array(
