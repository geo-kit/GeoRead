...
def egg_binary_data(egg_model: Path):
    ...
    return georead.binary.load(egg_model)


def test_load_egg(egg_model: Path):
    """Test loading Egg model binary data."""
    binary_data = georead.binary.load(egg_model)
    assert isinstance(binary_data, georead.binary.BinaryData)


def test_all_parts_exist(egg_binary_data: georead.binary.BinaryData):
    """Test that all existing files were loaded."""
    extensions: set[georead.binary.FileType] = {
        'EGRID',
        'INIT',
        'UNSMRY',
        'SMSPEC',
    ...
    for ext in extensions:
        assert isinstance(egg_binary_data[ext], georead.binary.BinaryFileData)


def test_find_unique(egg_binary_data: georead.binary.BinaryData):
    """Test that `BinaryFileData.find_unique` correctly finds unique element."""
    assert egg_binary_data['EGRID'].find_unique('GRIDHEAD') == 1


def test_find_unique_no_value(egg_binary_data: georead.binary.BinaryData):
...
def test_find_unique_error(egg_binary_data: georead.binary.BinaryData):
...
def test_value(egg_binary_data: georead.binary.BinaryData):
...
def test_seek(egg_binary_data: georead.binary.BinaryData):
...
def test_find(egg_binary_data: georead.binary.BinaryData):
...
def test_find_all(egg_binary_data: georead.binary.BinaryData):
    """Test `BinaryFileData.find_all`."""
    assert egg_binary_data['UNSMRY'].find_all('PARAMS') == [i for i in range(3, 364, 3)]


def test_find_prev(egg_binary_data: georead.binary.BinaryData):
